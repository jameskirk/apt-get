...
public class WinRegistry {
	...
	public static List<WindowsRegistryEntry> getRegistryEntriesWindows() throws InternalException {
		List<WindowsRegistryEntry> registryEntries = new ArrayList<WindowsRegistryEntry>();
		try {
			String registryUninstall64 = "Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall";
			List<String> programs = WinRegistry.readStringSubKeys(WinRegistry.HKEY_LOCAL_MACHINE, registryUninstall64);
			for (String program : programs) {
				String displayName = WinRegistry.readString(WinRegistry.HKEY_LOCAL_MACHINE,
						registryUninstall64 + "\\" + program, "DisplayName");
				String displayVersion = WinRegistry.readString(WinRegistry.HKEY_LOCAL_MACHINE,
						registryUninstall64 + "\\" + program, "DisplayVersion");
				if (displayName != null && !displayName.isEmpty()) {
					WindowsRegistryEntry registryEntry = new WindowsRegistryEntry(displayName, program, displayVersion);
					registryEntries.add(registryEntry);
					// Logger.debug(registryEntry.toString());
				}
			}
			
			registryUninstall64 = "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall";
			programs = WinRegistry.readStringSubKeys(WinRegistry.HKEY_LOCAL_MACHINE, registryUninstall64);
			for (String program : programs) {
				String displayName = WinRegistry.readString(WinRegistry.HKEY_LOCAL_MACHINE,
						registryUninstall64 + "\\" + program, "DisplayName");
				String displayVersion = WinRegistry.readString(WinRegistry.HKEY_LOCAL_MACHINE,
